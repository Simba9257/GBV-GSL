...
def load_data2(data_path, ptb_path, dataset, args):
    #adj = pickle.load(open(f'../data/graphs/{dataset}_adj.pkl', 'rb'))
    if args.ptb_feat == False:
        features = pickle.load(open(f'../dataset/{dataset}/{dataset}_features.pkl', 'rb'))
    else:
        features = sp.load_npz(ptb_path+"feat/"+dataset+"/"+"feat_"+str(args.ratio)+".npz")

    labels = pickle.load(open(f'../dataset/{dataset}/{dataset}_labels.pkl', 'rb'))
    data_mask = pickle.load(open(f'../dataset/{dataset}/{dataset}_tvt_nids.pkl', 'rb'))
    ...
    return DataSet(dataset=args.dataset, x=features, y=labels, view1=ori_view1, view2=ori_view2,
                   view1_indice=ori_view1_indice, view2_indice=ori_view2_indice,
                   idx_train=train_mask, idx_val=val_mask, idx_test=test_mask)
